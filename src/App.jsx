...
export default function App() {
  ...
  return (
    <div className="mp-app mp-body">
      <div className="synth-sky" aria-hidden="true">
      ...
      <style>{`
        .mp-app {
          ...
          position: relative;
          overflow-x: hidden;
          background: linear-gradient(180deg, var(--night) 0%, #2a1150 40%, #4a1246 72%, var(--night) 100%);
          color: var(--text);
          min-height: 100vh;
        ...
        .synth-sky { position: fixed; top: 0; left: 0; right: 0; height: 420px; pointer-events: none; z-index: 0; overflow: hidden; }
        .synth-sun { position: absolute; left: 50%; top: 30px; width: 280px; height: 280px; transform: translateX(-50%); border-radius: 50%; background: radial-gradient(circle at 50% 42%, var(--sun-core) 0%, var(--sun-mid) 35%, var(--magenta) 72%, transparent 100%); opacity: 0.85; }
        .synth-sun::after { content: ""; position: absolute; inset: 0; border-radius: 50%; background-image: repeating-linear-gradient(0deg, transparent 0 6px, rgba(18,8,31,0.88) 6px 11px); }
        .synth-horizon { position: absolute; left: 0; right: 0; top: 300px; height: 2px; background: linear-gradient(90deg, transparent, var(--cyan), var(--magenta), transparent); box-shadow: 0 0 24px 4px rgba(255,61,129,0.45); }
                  ...
                  {filteredLibrary.map((m) => (
                    ...
                    >
                      ...
                      <div className="suggested-type">
                        {MEAL_TYPE_META[m.mealType].label}
                      </div>
                    </button>
                  ))}
